...
>
> 本文档是项目说明书与开发基线（v0.2）。它覆盖了原 v0.1 草案，并据此前内审做了可行性收缩：只保留能做成、能做成对照、且有机会超过 SignNet / RWSE 的部分。
...
原草案的数学诊断（外积不能吸收符号、局部层只能看到 `|x_i|`、简并会把歧义群升级成 `O(m)`）是对的，必须保留。原草案的产品形态不可行：创新点被拆成三件已发表工作的拼接，主实验选错数据，图像与动态图会稀释唯一站得住的假设。

### 模块可行性

| 模块 | 可行性 | 对最终效果的作用 | 结论 |
|---|---|---|---|
| 相对谱核注意力偏差 `K = U g_θ(Λ) Uᵀ` | 高 | **最大性价比**。天然符号不变，简并时退化为谱投影，直接给注意力结构先验 | **默认开启** |
| SignNet 一阶谱场（沿用） | 高 | 绝对 PE 的合格下限；不复现它就无法声称改进 | **必做基线，内嵌于完整模型** |
| 简并分块 / 子空间投影 | 高 | ZINC 约 64% 的图有高维特征空间；丢掉块内信息会伤害主实验 | **默认开启** |
...
| `svec(\|S_i\|)` 局部二次基 | 高 | 理论信息不超出 `|x_i|`，MLP 可学；最多当轻量变体 | **Lite 可选，不当主贡献** |
...
孤立点令 `D_{ii} ← D_{ii} + \delta`（`δ ≈ 10^{-6}`），避免 `D^{-1/2}` 失败。`L_{\mathrm{sym}} = U Λ Uᵀ`，`0 = λ_1 ≤ ⋯ ≤ λ_N`。连通图上 `λ_1 = 0` 的模是 `D^{1/2}𝟙`，几乎不含位置信息，**默认丢掉零空间，取最小的 k 个正特征值**。多连通分支时零空间维数等于分支数，按简并块处理，不要假装它是一组独立的 PE 坐标。
...
**证明.** 取 `D_x = \mathrm{diag}(\mathrm{sign}(x))`（零分量任意），则 `f(x)=f(|x|)`。反向显然。∎
...
|---|---|---|---|---|
| 相对谱核（优先） | `K_{ij}^{(g)} = Σ_p g(λ_p)\, u_{pi} u_{pj}` | 每项 `s_p^2=1`，不变 | 整块求和即 `(g(λ_B) P_B)_{ij}` | 注意力偏差 |
| 一阶谱场 | `u_p ∈ ℝ^N` | 全局乘 `±1` | 块内坐标无意义，改用 `P_B` 的列/对角 | SignNet 绝对 PE |
| 二阶谱场 | `w_{pq} = u_p ∘ u_q` | 全局乘 `s_p s_q` | 块内 pair 无定义，必须丢弃或改投影 | 本项目 H1 |

**相对核（推荐写成滤波核）**

$$K^{(g)} = U\,\mathrm{diag}(g_\theta(\lambda_1),\dots,g_\theta(\lambda_k))\,U^\top,\qquad b_{ij} = \alpha\, K^{(g)}_{ij}.$$
...
| 滤波谱核 `K^{(g)}` | 是 | 用 `P_B` 则是 | 相对几何 / 热核 | **默认开** |
...
Data ─预计算─ L_sym 的 top-k (λ, U)，简并分块
                │
                ├─ 相对 SE:  K^{(g)}_h = U g_θ^{(h)}(Λ) Uᵀ   →  每头注意力偏差
                │
                ├─ 绝对 PE:   SignNet^{(1)}(U)                 →  z⁽¹⁾
                │
...
GPS layer: GatedGCN（局部） + Transformer（全局，attn += α_h K_h）
```
...
```
attn_h += α_h * K_h     # α_h 可学习标量，K_h 预计算或每次用当前 g_θ 重构
```

`K_h` 不进计算图对 `U` 的反传。`g_θ` 参数量保持很小（每头 < 32）。
...
- `spectral.precompute`：拉普拉斯、分块、缓存 `λ, U, block_ids`。
- `spectral.kernel`：`g_θ` 与 `K_h`。
...
5. 二阶场的负对照：逐点 `ψ`（预期 ≈ Kern，若显著更好则实验有 bug 或参数没匹配）。
6. 简并：关掉分块 vs 默认分块（ZINC 上应能看出差异）。
7. `L_sym` vs `L_comb`。
8. 不要把 `Y=g(L)X` 塞进这条链。
...
| 相对 / 稳定 PE | PEG、EquivStableLapPE、SPE | 近亲；谱核是滤波后的截断热核 |
| RWSE | Dwivedi et al. / GPS | 必须打赢或讲清何时谱优于随机游走 |
...
| 简并处理过度 / 不足 | ZINC 大量近简并 | 相对间隙 + 投影；单测随机 `Q` |
...
时间分配建议：约一半给 v0.1–v0.2 的复现与测试。没有这条，后面的模块没有解释权。
...
- Wang et al., *Equivariant and Stable Positional Encoding for More Powerful Graph Neural Networks*, ICLR 2022.
- Huang et al., *On the Stability of Expressive Positional Encodings for Graphs (SPE)*, 2024.
